...
$truncate="TRUNCATE TABLE $userid";
...
if(isset($_POST['app']) || isset($_POST['num']) || isset($_POST['rat'])){
	//$app= implode(',', $_POST['app']);
	//$num= implode(',', $_POST['num']);
	//$rat= implode(',', $_POST['rat']);
	$app=$_POST['app'];
	$num=$_POST['num'];
	$rat=$_POST['rat'];
	for($x=0;$x<count($num);$x++){
		...
		$qty=$num[$x];
		$rating=$rat[$x];
		$add="INSERT INTO $userid(Appliance,Num,Rating) VALUES ('$appliance','$qty','$rating')";
