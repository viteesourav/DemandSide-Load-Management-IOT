...
<div class="container">
	<div class="boxbox">
     ...
     $sum="SELECT SUM(Num*Rating) FROM $userid WHERE Rating<=75";
     $sum1=mysqli_query($con,$sum);
     $row=mysqli_fetch_assoc($sum1);
     $sum1=intval($row['SUM(Num*Rating)']);
     $sum="SELECT SUM(Num*Rating) FROM $userid WHERE Rating>75 AND Rating<=200";
     $sum2=mysqli_query($con,$sum);
     $row=mysqli_fetch_assoc($sum2);
     $sum2=intval($row['SUM(Num*Rating)']);    

     $loaddisplay= intdiv($load*$fract*1000,30*27);
     if($batpower<=1200)
     {
     	$battery=0;
     } 
     ...
     {
     	$p1=round($sum1-$battery);
     }

     if($p1==0)
     {
     	if(($sum1*0.1+$sum2*0.5)<=$battery)
     		$p2=0;
     	else
     		$p2=round(($sum1*0.1+$sum2*0.5)-$battery);
     }
     else
     {
     	$p2=round($loaddisplay*0.1);
     }

     if($p1==0 && $p2==0)
     {
     	$p3=round($loaddisplay*0.25);
     	$p4=round($loaddisplay*0.35);
     	$p5=round($loaddisplay*0.4);
     }
     else if($p1==0 && $p2!=0)
     {
     	$p3=round(($loaddisplay-$p2)*0.25);
     	$p4=round(($loaddisplay-$p2)*0.35);
     	$p5=round(($loaddisplay-$p2)*0.4);
     }
     else
     {
     	$p3=round($loaddisplay*0.2);
     	$p4=round($loaddisplay*0.3);
     	$p5=round($loaddisplay*0.4);
     }
     
     $maxp1=round($loaddisplay*0.5);     
     $maxp2=round($loaddisplay*0.5);     
...
<div class="container">
	<div class="boxbox1">
		...
		<form action="adduserloadtodb.php" method="post">
			...
			<div class="row">
				<div class="col-md-5 formleft">
					<label><font size="4">Medium Loads (Priority 3): </font><font color="red" size="2">(Range: 0 W to <?php echo round($maxp2); ?>W)</font></label>
				...
				<div class="col-md-5 form-right">
					<label><font size="4">Medium Loads (Priority 4): </font><font color="red" size="2">(Range: 0 W to <?php echo round($loaddisplay); ?>W)</font></label>
						<input type="number" name="m2" id="m2" min="0" max="<?php echo round($loaddisplay); ?>" value="<?php echo $p3; ?>" class="form-control" required placeholder="Enter M2" 
						onblur="update()">
				</div>
			</div>
			<br>
			...
			<div align="center">
				<table class="table-content">
					...
					$userid=$_SESSION['userid'];
					$qry= "SELECT id,Appliance, Num, Rating FROM $userid WHERE Rating <= 75 ORDER BY Rating";
					$result=mysqli_query($con,$qry);
					...
					// Get users from the database
					$userid=$_SESSION['userid'];
					$qry= "SELECT id,Appliance, Num, Rating FROM $userid WHERE Rating>75 AND Rating<=200 ORDER BY Rating";
					$result=mysqli_query($con,$qry);
					...
					// Get users from the database
					$userid=$_SESSION['userid'];
					$qry= "SELECT id,Appliance, Num, Rating FROM $userid WHERE Rating>200 AND Rating<=550 ORDER BY Rating";
					$result=mysqli_query($con,$qry);
					...
					// Get users from the database
					$userid=$_SESSION['userid'];
					$qry= "SELECT id,Appliance, Num, Rating FROM $userid WHERE Rating>550 AND Rating<=900 ORDER BY Rating";
					$result=mysqli_query($con,$qry);
					...
					// Get users from the database
					$userid=$_SESSION['userid'];
					$qry= "SELECT id,Appliance, Num, Rating FROM $userid WHERE Rating>900 ORDER BY Rating";
					$result=mysqli_query($con,$qry);
       ...
       function alertme(){
	        ...
	        function check2()
        	{
	       		...
	       		if(<?php echo $battery; ?>==0)
	       			sum12=0;
	       		document.getElementById("l1").value=sum12+sum13;
	       		...
	       		if(total>50 || total<-50 || l1<-3 || l1><?php echo $battery; ?>)
				{
					document.getElementById("finally").disabled=true;
				}
				else
	        ...
	        function check3()
        	{
	       		...
	       		if(<?php echo $battery; ?>==0)
	       			sum13=0;
	       		document.getElementById("l1").value=sum12+sum13;
	       		document.getElementById("m1").value=sum34+sum43;
	       		if(parseInt(document.getElementById("l1").value)><?php echo $battery; ?>)
	       			document.getElementById("l1").value=<?php echo round($battery); ?>;
	        ...
	        function check5()
        	{
	       		...
	       		var arr4=$("select[name='prior5[]']").map(function(){return $(this).val();}).get();
	       		var i;
	       		sum56=0;
	       		sum66=0;
	       		for (i=0; i<arr1.length; i++)
	       		...
	       		document.getElementById("h1").value=sum55+sum56;
	       		document.getElementById("h2").value=sum65+sum66;
	        ...
	        function check6()
        	{
	       		...
	       		var arr4=$("select[name='prior6[]']").map(function(){return $(this).val();}).get();
	       		var i;
	       		sum55=0;
	       		sum65=0;
	       		for (i=0; i<arr1.length; i++)
	       		...
	       		document.getElementById("h1").value=sum55+sum56;
	       		document.getElementById("h2").value=sum65+sum66;
	       		if(parseInt(document.getElementById("h2").value)><?php echo $loaddisplay; ?>)
	       			document.getElementById("h2").value=<?php echo round($loaddisplay); ?>;
